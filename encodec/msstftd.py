...
class DiscriminatorSTFT(nn.Module):
    ...
    def __init__(self, filters: int, in_channels: int = 1, out_channels: int = 1,
        ...
        self.spec_transform = torchaudio.transforms.Spectrogram(
            ...
            normalized=normalized, center=False, pad_mode=None, power=None
        )
        spec_channels = 2 * self.in_channels
        self.convs = nn.ModuleList()
        self.convs.append(
            NormConv2d(spec_channels, self.filters, kernel_size=kernel_size, padding=get_2d_padding(kernel_size), norm=norm)
        )
...
class MultiScaleSTFTDiscriminator(nn.Module):
    ...
    def __init__(self, filters: int, in_channels: int = 1, out_channels: int = 1,
        ...
        self.discriminators = nn.ModuleList([
            DiscriminatorSTFT(filters, in_channels, out_channels, n_ffts[i], hop_lengths[i], win_lengths[i], **kwargs)  
